...
\begin{equation}
    {P}_T(k)=\frac{32\pi G}{a^3H}\bigg(\frac{k}{aH}\bigg)^{-2\nu_T},\qquad\mathcal{P}_T(k)\defeq\frac{k^3}{2\pi^2}{P}_T(k)=\frac{2H^2}{\pi^2M_\text{pl}^2}\bigg(\frac{k}{aH}\bigg)^{3-2\nu_T},\label{eq:tensor_power_spectrum}
\end{equation}
...
\label{fig:PS_comp}
\end{subfigure}
\caption{Plots of the power spectra presented in this section. The figure on the left (a) shows the behavior of the bump as $\sigma$ increases: note how a plateau is produced for high values of $\sigma$. The plot on the right (b) compares the power law (dotted) with the bump (dashed) and with their sum (solid line). }
\end{figure}
