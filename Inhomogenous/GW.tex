...
\begin{tikzpicture}
  ...
  \end{axis}
\end{tikzpicture}
\caption{Comparison of the evolution of the amplitude of primordial gravitational waves in the radiation dominated universe and in the matter dominated universe. We used the condition $C_2=0$ and $C_1=1$. }
\label{fig:h_m/r}
\end{figure}
...
$$F_{\mu\nu}= A_{\nu;\mu} -  A_{\mu;\nu}= A_{\nu,\mu} -  A_{\mu,\nu},$$
where $A_\mu$ is the \emph{4-potential} and we turned covariant derivatives into partials since all the Christoffel symbols cancel out. To obtain the \emph{electric} and \emph{magnetic fields} in a curved spacetime, a specific time direction must be chosen: fixing a time-like vector $u^\mu$ then we define
\begin{equation}
...
Note that in Minkowski spacetime $u^\mu=(1,0,0,0)$, which gives the usual definitions $E_i=F_{i0}$ and $B_i=\epsilon_{ijk}F^{jk}/2$. The same holds in FRW spacetime with cosmic time, however using conformal time an extra factor $a^{-1}$ appears due to the normalization $u^\mu u_\mu=-1$. Lastly, inverting equations \eqref{eq:EB_fields} we can show that the electromagnetic tensor is
\begin{equation}
...
\end{equation}
were $\mathcal P_B$ is the power spectrum of the magnetic field. Phenomenologically these power spectra can be characterized by a power law
$$\mathcal{P}_B(k)=\mathcal A_B\bigg(\frac{k}{k_0}\bigg)^{n_B},$$
which then allows for the 2-points correlation function of the anisotropic stress to be obtained:
...
and with $P_{ij}\defeq \delta_{ij} - \hat k_i\hat k_j.$
...
$$
In the above, the damping scale $k_D$ appears: mathematically it corresponds to an upper cut-off to every momentum integral of the power spectrum.
