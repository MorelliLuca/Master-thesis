...
\begin{figure}
  ...
  \foreach \i in {0,...,3} {
        \draw[gray, ultra thick, dashed] (0,1+\i*0.85) -- (3,0.5+\i*0.7);
  }
  \draw[gray, ultra thick, dashed] (1,4) -- (3,0.5+4*0.7);

  % 5 points on the left vertical axis
  \foreach \i in {0,...,3} {
    \fill[gray] (0,1+\i*0.85) circle (2pt);
  }

  \foreach \i in {0,...,4} {
    \fill[gray] (3,0.5+\i*0.7) circle (2pt);
  }

  
  % Labels for the points
  \node[right] at (0,.8) {$t_1$};
  \node[left] at (3,0.3) {$t_0$};
  \foreach \i in {0,...,2} {
