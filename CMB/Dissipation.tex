...
where $\mathcal{J}_\mu(z)$ is the branching ratio that can be either \eqref{eq:BR_soft} or \eqref{eq:BR_soft_soft}.
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    \addlegendentry{J. Chluba}
    \addplot [thick, smooth] table {CMB/mu_Window_soft.dat};
    \addlegendentry{Improved window}
  \end{axis}
\end{tikzpicture}
\caption{Plot of the window function of the $\mu$ distortion sourced by the dissipation of primordial gravitational waves. A comparison between the window function obtained by J. Chluba in \cite{Chluba_tens_diss} and the one we obtained with a better approximation of the branching ratio is shown.}
\label{fig:mu_window}
\end{figure}
