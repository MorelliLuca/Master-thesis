...
\end{equation}
where $W_a$ is the window function of the distortion $a$. This decomposition allows us to better understand at which scales perturbation dissipation more efficiently sources distortions. Inserting the heating rate we obtained in the previous section in the integral \eqref{eq:SD_amplitude_branching}, we find the actual full expression for the $\mu-$distortion generated by primordial gravitational waves
...
where $\mathcal{J}_\mu(z)$ is the branching ratio that can be approximated either by \eqref{eq:BR_soft} or \eqref{eq:BR_soft_soft} and $\mathcal T_h$ must change with $z$ as we explained in the last section.
...
\begin{align*}
    ...
    \end{cases},
\end{align*}
where we included the expressions for the branching ratios and we recall that $z_\text{th}\approx 2\times 10^6$ while $z_{\mu y}\approx 5\times 10^4$.\\
The first branching ratio has been used in the work of Jens Chluba \cite{Chluba_tens_diss}, a comparison between the two resulting window functions is shown in Figure \ref{fig:mu_window}: as we can see, using the second branching ratio the window function covers a wider range of scales, reaching much bigger scales of the order of $k\approx10^{-5}$ Mpc$^{-1}$. On the other hand the small scale behavior remains the same as the one obtained by Chluba \cite{Chluba_tens_diss}: primordial gravitational waves at the scale of the order $k\approx10^{8}$ Mpc$^{-1}$ sources $\mu$-distortions. The new contribution to the window function that we obtained comes from the energy deposited in the plasma during the $\mu$-y transition, when both distortions can be sourced. This also explains why this happens only at large scales, which corresponds to the perturbations that reentered the comoving Hubble horizon later in time.
\begin{figure}
    \centering
\begin{tikzpicture}
  \begin{axis}[
    ...
    \addlegendentry{J. Chluba window}
    \addplot [thick, smooth] table {CMB/mu_Window_soft.dat};
    \addlegendentry{Improved window}
  \end{axis}
\end{tikzpicture}
\caption{Plot of the window function of the $\mu$ distortion sourced by the dissipation of primordial gravitational waves. A comparison between the window function obtained by J. Chluba in \cite{Chluba_tens_diss} and the one we obtained with a softer $mu$-y transitioning branching ratio is shown.}
\label{fig:mu_window}
\end{figure}

The window function for the y-distortion is instead obtained in the same way just by using the branching ratios corresponding to the y-distortions:
\begin{align*}
...
\subsection{Theoretical analysis of the distortions}
We conclude this chapter by discussing our predictions on the spectral distortions sourced by the dissipation of primordial gravitational waves. To obtain these results we modified the {distortion} module of CLASS so that the heating rate we obtained is computed numerically and then used by CLASS to calculate the spectral distortions amplitudes.


To begin with, we tried to reproduce the results that Chluba et al. obtained in \cite{Chluba_tens_diss}: this was initially used as a benchmark to understand whether our implementation works correctly.
\begin{table}[b]
\begin{tabular}{llllll}

                                                                                                                   &                    & Chluba et al.       & Sharp tr.    & Smooth tr.  & Exact \\
\hline
\multirow{3}{*}{\begin{tabular}[c]{@{}l@{}}$\mathcal A_T=2.2\times10^{-10}$\\ $k_0=0.05$ Mpc$^{-1}$\end{tabular}}  & $n_t=0$            & $1.8\times10^{-14}$ & $1.3\times10^{-14}$ & $2.1\times10^{-14}$& $2.5\times10^{-14}$ \\
...
\hline
\multirow{3}{*}{\begin{tabular}[c]{@{}l@{}}$\mathcal A_T=2.4\times10^{-10}$\\ $k_0=0.002$ Mpc$^{-1}$\end{tabular}} & $n_t=0$            & $1.9\times10^{-14}$ & $1.4\times10^{-14}$ & $2.4\times10^{-14}$&$3.4\times10^{-14}$ \\
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    \addlegendentry{Soft transitioning}
